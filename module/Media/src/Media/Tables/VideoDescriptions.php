...
class VideoDescriptions
{
	...
	{
		return $this->getTablegateway()->select(array('video_id' => $videoId));
	}
	...
	{
		if(!is_array($description) && !($description instanceof \Media\ArrayObject\VideoDescription))
			throw new \Exception('Data must be array or Media\ArrayObject\VideoDescription');
	...
	public function edit($description, $where = array())
	{
		...
		if(!is_array($description))
		{
			throw new \Exception('Data must be array or Pages\ArrayObject\VideoDescription');
		}
